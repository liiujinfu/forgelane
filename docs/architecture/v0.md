...
v0 provider sync should be designed around three paths:
...
Signals are provider feedback attached to a ChangeSet. In v0 they can be read
...
v0 can start with `start`, `stop`, `retry`, `request_changes`, and `close`.
`approve`, `merge`, and richer reassignment can stay behind explicit product
decisions because they touch team policy more directly.
...
Workspace
  ...
  owns temporary credentials for the run
...
- artifact directory for patches, summaries, screenshots, or debug files;
...
- provider webhooks when reachable, polling/manual refresh when not;
...
- commit list;
- CI/check and review signals;
- control actions such as stop, retry, request changes, and close.
...
This is the first slice that mutates GitHub change state. It should stay limited
to branch push and draft PR creation/update. Review comments, CI/check sync,
requested-changes follow-up runs, and approval handling belong to later v0.1+
slices.
