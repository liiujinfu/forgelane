...
Issue #3 should still create the `events` table and append compact WorkItem
import/refresh Events transactionally with WorkItem writes, but a generic
`forgelane events list` CLI can be split into a later event-inspection issue.
The import command output and store tests can prove Event creation for this
slice. Even without a generic event browser, the table should include nullable,
indexed `work_item_id` and `forge_project_id` association columns so tests and
later event-inspection flows can query events without parsing JSON payload.
