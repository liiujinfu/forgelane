...
Repository initialization should be idempotent when the existing ForgeProject
matches the requested value. If an existing config points at a different
ForgeProject, initialization should fail unless the caller passes an explicit
non-interactive replacement flag such as `--force`.
The first local config path can be `.forgelane/repository.json` in the current
ForgeLane local context. Treat this as local control-plane configuration by
default, not target source-repository data intended for commit.
...
The first v0 tracer bullet should establish the pre-run WorkItem import path
before invoking agents, creating AgentRuns, or mutating provider change state:
...
This slice should not yet create an AgentRun, create a RunSpec, run the agent,
push commits, or create a draft PR. Its purpose is to prove that provider-owned
WorkItem data can be referenced, snapshotted, stored separately from execution
state, audited with an Event, and observed from the CLI.
...
forgelane events list --work-item github://github.com/owner/repo/issues/123
forgelane events list --issue 123
forgelane events list --work-item-id <local_work_item_id>
```
...
`work-items show` and `events list` should make provider identity the primary
user-facing lookup path. Full ProviderRefs work without repository config.
`--issue 123` is available only when repository config can resolve it. Explicit
local id flags such as `--id` and `--work-item-id` are allowed as debugging and
fallback paths, but users should not need to remember internal WorkItem ids.
...
external_id           # provider id or node id when available
...
ProviderRef append `work_item.refreshed`. Event payloads can include compact
fields such as ProviderRef, repository ref, provider update time, and local
WorkItem id. Failed parsing, provider reads, and rolled-back transactions should
not write Events in this slice; successful WorkItem upsert and Event append
must happen in the same SQLite transaction.
...
failures. Comments and GitHub mutations are out of scope.
...
`agent_runs`, `run_specs`, `log_chunks`, `change_sets`, `runner_jobs`, and
`workspaces` tables until the next slices actually start runs, capture process
logs, lease workspaces, push commits, and open a draft PR.
