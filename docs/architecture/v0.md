...
The repository currently implements the CLI-first local control spine through
Workspace preparation:

- `forgelane init` persists an initialized ForgeProject in the instance-global
  SQLite store at `~/.forgelane/forgelane.db`.
- `forgelane work-items import` reads a GitHub issue through the provider
  boundary, stores a cached WorkItem snapshot, and appends a compact
  `work_item.imported` or `work_item.refreshed` Event.
- `forgelane work-items show` reads only the cached local snapshot.
- `forgelane runs create` creates a new planned AgentRun, succeeded `start`
...
- `forgelane runs show` and `forgelane events list --run` inspect local SQLite
  state without contacting providers.

The code does not yet execute the generic command AgentAdapter, capture agent
stdout/stderr as log chunks, materialize commits, push branches, or create
draft PRs/MRs.
...
## Implemented WorkItem Import Slice

The first v0 tracer bullet established the pre-run WorkItem import path before
...
The current SQLite schema has moved beyond the first import-only cut. It now
includes:
...
`events` uses the provider-neutral shape defined above and includes nullable,
indexed association columns for WorkItems, AgentRuns, ControlActions, and
ForgeProjects. `log_chunks`, `change_sets`, commit-ref tables, and provider
snapshot tables are still deferred until the slices that capture process logs,
push commits, sync provider signals, and open draft PRs.

## Current AgentRun and Workspace Preparation Slice

The second v0 tracer bullet is partially implemented. The current CLI proves
AgentRun planning and Workspace preparation without executing an agent process
or mutating provider change state:
...
This slice still needs the agent execution half of the runner boundary:
invoking the generic command AgentAdapter, capturing stdout/stderr as log
chunks, recording terminal status, and supporting timeout/cancel outcomes. It
must still avoid pushing commits or creating draft PRs until the ChangeSet
slice.
...
Implemented schema for this slice:
