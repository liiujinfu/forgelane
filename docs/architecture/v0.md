...
- ChangeSet: the branch, PR, and commits produced or updated by the run.
...
- manages provider-owned branch and PR side effects;
- creates or updates draft PRs only after workflow decides there is a
...
- does not push provider branches or create PRs directly.
...
- WorkItem Provider: reads and comments on task sources such as GitHub Issues or
  GitLab Issues.
- Change Provider: prepares branches, creates draft PRs, reads change
...
issues, PRs, commits, reviews, comments, checks, and CI status. ForgeLane
...
Provider-owned:
  ...
  PR title/body/status/reviews/comments
  CI/check status
  merge/close state
...
It is not just a provider PR, but the ForgeLane-owned record that connects a
WorkItem, branch, PR, commits, and external feedback signals.
...
ChangeSet
  ...
  has one branch
  has zero or one draft PR at first
  has many commits
  has many signals
...
append commits to the existing branch and draft PR.
...
outputs. It does not mean the PR was approved, merged, or even correct.
...
- runner: job created, workspace prepared, agent process started, agent process
  exited
- change set: branch created, draft opened, commit detected, PR updated,
  closed, merged
...
close             -> ChangeSet     -> closes provider PR when allowed
...
- decide whether a PR should be merged;
- own approval policy;
...
draft PR creation belong to the workflow/change-provider layer, not to the
agent process.
...
must not create an empty commit or draft PR. A more isolated future runner
...
- ChangeSet branch, draft PR link, and current change status;
...
8. After the first commit is pushed successfully, the workflow layer uses the
   Change Provider to create or update a draft PR for the branch.
9. ForgeLane shows run state, events, logs, branch, and PR link.
...
provider PR, #3 should reject it with a clear user-visible
error rather than storing it as a WorkItem. In v0, issues/tickets/tasks are
WorkItem inputs; PRs are ChangeSet delivery artifacts or later review/fix
...
to the existing draft PR after review feedback or failure recovery.
...
human closes or edits the PR outside ForgeLane
```
...
- draft PR creation failure: keep branch and commits as ChangeSet state,
  mark ChangeSet below `draft_open`, allow retrying PR creation.
